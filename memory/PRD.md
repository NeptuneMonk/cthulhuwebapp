...
- **P0: Unified Treasury Wallet** — Removed separate "Wallet" tab from admin. Treasury is now the single wallet hub for all on-chain operations (etching, releases, snapshot announces). Added WIF import directly in Treasury panel. Updated all UI references. Etch Manager explicitly notes it uses Treasury wallet. Auto-delta vacuum controlled by `AUTO_VACUUM_ENABLED` env var (default off).
